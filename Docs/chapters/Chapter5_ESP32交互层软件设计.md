...
![图 5-1 ESP32 主节点任务架构](../images/ai_generated/cropped//esp32_task_flow.png)
...
DeepSeek 大模型集成模块（`deepseek_api.cpp`）封装了与 DeepSeek Chat API 的完整交互流程，如图 5-2 所示。API 调用采用非流式模式，通过 `HttpClient` 类发送 HTTPS 请求，`messages` 数组由系统提示词、历史对话和当前用户输入组成，支持多轮对话上下文延续。

**图 5-2 DeepSeek API 调用流程**

![图 5-2 DeepSeek API 调用流程](../images/ai_generated/cropped//deepseek_api_flow.png)
...
![图 5-3 语音助手状态机](../images/ai_generated/cropped//voice_assistant_state_machine.png)

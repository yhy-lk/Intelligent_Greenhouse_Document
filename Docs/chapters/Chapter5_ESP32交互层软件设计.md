...
图形界面基于 LVGL v9.3.0 框架实现，界面布局采用 NXP GUI Guider 可视化设计工具生成页面骨架代码，自定义业务逻辑在 `custom.c` 中手动补充。显示驱动通过 `display_hal` 模块与 LovyanGFX 库对接，驱动 SPI 接口的 240×320 TFT 彩色触摸屏。
...
CAN 网络服务（`can_network_service.c`）封装了 ESP32-S3 的 TWAI 驱动，实现了 CAN 总线的硬件初始化、报文非阻塞轮询接收和协议解包。CAN 总线的硬件电路设计详见第 3 章 3.5 节，帧格式与协议编解码逻辑详见第 4 章 4.4 节。本系统采用 1Mbps 波特率以满足多节点高频数据上报需求。

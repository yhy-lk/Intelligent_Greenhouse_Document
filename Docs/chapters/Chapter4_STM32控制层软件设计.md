...
土壤湿度传感器输出 0～3.3 V 模拟信号，由 ADC1（12-bit）在 PC1 引脚采样，并通过内部参考电压 VREFINT 校准供电波动。湿度百分比通过线性映射获得：

$$H_{soil} = \text{clamp}\left(\frac{4000 - D_{ADC}}{4000 - 1000} \times 100,\ 0,\ 100\right)$$

其中干燥状态 ADC 约 4000 对应 0%，饱和状态 ADC 约 1000 对应 100%，阈值通过实际标定获得。域任务 `soil_moisture_task` 采用循环连续采样。

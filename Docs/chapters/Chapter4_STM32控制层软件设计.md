...
作为分布式系统的基础执行单元，STM32 控制层承担着环境数据高频采集与底层机构调节的核心工作。本章立足于 Embassy 异步运行时框架，解析节点软件系统的四层解耦架构，探讨传感数据处理、增量式 PID 闭环控制算法以及 CAN 总线协议栈帧调度的设计细节，以期验证底层高频调控逻辑的可靠性与有效性。
...
::: {custom-style="图片"}
![](../images/draw/png/stm32_task_overview.drawio.png)
:::
::: {custom-style="表题"}
...
::: {custom-style="图片"}
![](../images/draw/png/pid_fan_control.drawio.png)
:::
::: {custom-style="表题"}
...
::: {custom-style="图片"}
![](../images/draw/png/dispatcher_logic.drawio.png)
:::
::: {custom-style="表题"}
图 4-3 迟滞调度器控制逻辑
:::
...
::: {custom-style="图片"}
![](../images/draw/png/can_rx_flow.drawio.png)
:::
::: {custom-style="表题"}
图 4-4 CAN 接收路由处理流程
:::

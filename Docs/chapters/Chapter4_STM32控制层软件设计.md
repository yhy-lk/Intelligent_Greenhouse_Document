...
![图 4-1 STM32 从节点异步任务派发关系](../images/code_generated/stm32_task_overview.png)
...
土壤湿度传感器输出 0～3.3 V 模拟信号，由 ADC1（12-bit）在 PC1 引脚采样，并通过内部参考电压 VREFINT 校准供电波动。湿度百分比按第 3 章 3.3 节给出的映射公式计算，域任务 `soil_moisture_task` 采用循环连续采样。
...
![图 4-2 通风风扇 PID 闭环控制流程](../images/code_generated/pid_fan_control.png)
...
![图 4-3 迟滞调度器控制逻辑](../images/code_generated/dispatcher_logic.png)
...
CAN 协议模块位于 `bsw/src/can_proto.rs`，定义了 11-bit 标准标识符的编解码逻辑。标识符构建公式为 $\text{CAN\_ID} = (\text{func\_code} \ll 7) \, | \, \text{node\_id}$，其中 4-bit 功能码定义帧类型（Alert、TimeSync、Write、Report），7-bit 节点 ID 标识地址。本从节点 ID 为 `0x01`，广播地址为 `0x00`。

浮点物理量采用缩放因子转换为整数传输：温度、湿度等采用 ×100 缩放，PID 参数采用 ×1000 缩放，整数量直接传输。
...
![图 4-4 CAN 接收路由处理流程](../images/code_generated/can_rx_flow.png)

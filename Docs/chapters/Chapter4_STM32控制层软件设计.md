...
![图 4-1 STM32 从节点异步任务派发关系](../images/ai_generated/cropped//stm32_task_overview.png)
...
数据完整性校验包含全零/全 0xFF 异常检测和 CRC-8 校验两层。校验通过后，按以下公式将 16-bit 原始值转换为物理量：
...
土壤湿度传感器输出 0～3.3 V 模拟信号，由 ADC1（12-bit）在 PC1 引脚采样，驱动位于 `firmware/src/bin/domains/soil_moisture.rs`。传感器浸水时输出低电压、干燥时输出高电压，因此采用反向线性映射将原始值转换为湿度百分比：

$$M = \frac{4000 - D_{ADC}}{4000 - 1000} \times 100\%$$

其中 $D_{ADC}$ 为 12-bit 原始采样值，4000 和 1000 分别对应干燥和浸水状态的经验校准值，结果限幅至 $[0, 100]$。域任务 `soil_moisture_task` 以 1 秒周期循环采样。
...
通风风扇是系统中唯一需要连续调节的执行器，采用离散位置式 PID 控制算法实现转速闭环[@ang2005pid; @astrom2006pid]。PID 控制器位于 `bsw/src/pid.rs`，其输出 $u(k)$ 由比例、积分、微分三项组成：
...
![图 4-2 通风风扇 PID 闭环控制流程](../images/ai_generated/cropped//pid_fan_control.png)
...
![图 4-3 迟滞调度器控制逻辑](../images/ai_generated/cropped//dispatcher_logic.png)
...
CAN 协议编解码模块位于 `bsw/src/can_proto.rs`，实现第 2 章 2.2 节所述的标识符构建与载荷编解码逻辑。本从节点 ID 配置为 `0x01`，广播地址为 `0x00`。除 ×100 缩放外，PID 参数采用 ×1000 缩放以保留更高精度。
...
![图 4-4 CAN 接收路由处理流程](../images/ai_generated/cropped//can_rx_flow.png)

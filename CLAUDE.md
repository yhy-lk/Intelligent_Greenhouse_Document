...
详细写作流程由 Skills 承载：`thesis-writer`（论文正文写作）、`abstract-writer`（英文摘要）、`mermaid-renderer`（图表渲染）、`thesis-reviewer`（论文审查）、`diagram-prompt-generator`（画图提示词）、`image-cropper`（AI 图片水印裁剪）、`image-selector`（多比例裁剪挑选）。
...
总计 9500-14500 字（不含代码、参考文献、附录）。**逐节撰写，禁止整章生成。**
